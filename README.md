...
- 目录
    ...
    + [4 余额主动更新相关功能](#4-----)
      ...
      - [4.3 订单状态查询](#43-----)
        ...
        * [4.3.2 返回参数](#432-----)
      - [4.4 附件](#44-----)
        * [4.4.1 资金变动类型action_type](#441-----)
        * [4.4.2 错误代码error_code](#442-----)
...
| userName      | string | 1 < length < 50 | 用户名 |
| sysUserName      | string | 1 < length <= 100 | 系统账号 |
| leagueNameCn     | string | 5 <= length <= 50 | 联赛名(中文)  |
| leagueNameEn     | string | 5 <= length <= 100 | 联赛名(英文)  |
| homeTeamNameCn     | string | 5 <= length <= 50 | 主队名(中文)  |
| homeTeamNameEn     | string | 5 <= length <= 100 | 主队名(英文)  |
...
<br>
您可以提供一个接口，确保接口名、传入、返回参数符合以下规范，我们将主动为您推送用户的余额变动情况。
<br>

> [!WARNING]
> 本部分并非我们提供接口，而是由您来提供接口，您制作完接口之后请将接口地址告知我们的对接人员，我们在系统中人工配置您的接口后方可使用。
...
#### <span id="441-----">4.4.1 资金变动类型action_type</span>

| 参数   | 资金变动类型     |
| ---- | -------- |
| 1    | 投注本金扣除   |
| 2    | 赢利本金返还   |  订单赢利，返还本金100  |
| 3    | 赢利   |    订单赢利，返还赢利20  |
| 4    | 赢利取消   |   赢利订单回滚，收回赢利20  |
| 5    | 赢利投注取消   |   赢利订单回滚，扣回订单本金100  |
| 6    | 投注本金返还   |
| 7    | 自行撤单   |
| 8    | 恢复订单   |
| 9    | 回收赢利   |  当已经派彩的订单被取消时，收回赢利20 |
| 10    | 回收本金   |  当已经派彩的订单被取消时，扣回订单本金100  |
| 11    | 下单失败   |
| 12    | 其他余额调整   |

#### <span id="442-----">4.4.2 错误代码error_code</span>
